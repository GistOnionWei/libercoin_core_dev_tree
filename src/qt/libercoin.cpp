...
{
    return QCoreApplication::translate("libercoin-core", psz).toStdString();
}
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. LIBERCOIN Core can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(libercoin);
    Q_INIT_RESOURCE(libercoin_locale);
    ...
    if(!ghostnodeConfig.read(strErr)) {
        QMessageBox::critical(0, QObject::tr("LIBERCOIN Core"),
