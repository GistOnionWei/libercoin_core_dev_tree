...
The RPM spec file provided here is for LIBERCOIN-Core 1.0.0 and builds on CentOS
...
reference the release tarball available from https://libercoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[LIBERCOIN git repository](https://github.com/libercoin/libercoin) but are not part of
...
Currently only `Source10` is used, for the example `libercoin.conf` file.
...
in the libercoin git repository. At present only an SVG version of the LIBERCOIN
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the libercoin
...
needed when 0.12.1 is released, a proper fix is already in the LIBERCOIN git
master branch.
...
The `libercoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `libercoin-server` package. At this point, most RPM based
...
The `libercoin-server` package should have SELinux support. How to properly do
...
LIBERCOIN-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/libercoin/). He has been
packaging LIBERCOIN for Fedora at least since 2012.
...
4. I renamed the `libercoin` package that contains the Qt GUI to `libercoin-core` as
that appears to be how the general population refers to it, in contrast to
`libercoin-xt` or `libercoin-classic`. I wanted to make sure the general population
